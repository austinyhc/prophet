...
class HSDMainView(Frame):
    def __init__(self, screen, hsd_info):
        ...
        layout_header.add_widget(Label("HSDatalog Command Line Interface", align=u'^'))
        ...
        if self._hsd_info_model.cli_flags.acq_name is not None and self._hsd_info_model.cli_flags.acq_name != "":
            ...
            layout_cli_flags.add_widget(Label("- Acquisition name: "))
        ...
        self._list_view = ListBox(
            Widget.FILL_FRAME,
            [("{}) [{}] - {}".format(i,dev.device_info.alias, dev.device_info.part_number), i) for i,dev in enumerate(dev_list)],
            name="device_list",
            add_scroll_bar=True,
        ...
        # Layout Device Info --------------------------------------------------------------------------------------------
        self._hsd_info_model.update_device_info()
        layout_device_info = Layout([100])
        self.add_layout(layout_device_info)
        if self._hsd_info_model.selected_device_info is None:
            ...
            dev_sn = "- sn: {}".format(self._hsd_info_model.selected_device_info.serial_number)
            dev_al = "- alias: {}".format(self._hsd_info_model.selected_device_info.alias)
            dev_pn = "- pn: {}".format(self._hsd_info_model.selected_device_info.part_number)
            dev_url = "- url: {}".format(self._hsd_info_model.selected_device_info.url)
            dev_fwn = "- fw_name: {}".format(self._hsd_info_model.selected_device_info.fw_name)
            dev_fwv = "- fw_version: {}".format(self._hsd_info_model.selected_device_info.fw_version)
            dev_dfe = "- data_file_ext: {}".format(self._hsd_info_model.selected_device_info.data_file_ext)
            dev_dff = "- data_file_format: {}".format(self._hsd_info_model.selected_device_info.data_file_format)
            dev_ns = "- n_sensor: {}".format(self._hsd_info_model.selected_device_info.n_sensor)
        ...
        layout_device_info.add_widget(self.dev_id_lbl)
        layout_device_info.add_widget(self.dev_sn_lbl)
        layout_device_info.add_widget(self.dev_al_lbl)
        layout_device_info.add_widget(self.dev_pn_lbl)
        layout_device_info.add_widget(self.dev_url_lbl)
        layout_device_info.add_widget(self.dev_fwn_lbl)
        layout_device_info.add_widget(self.dev_fwv_lbl)
        layout_device_info.add_widget(self.dev_dfe_lbl)
        layout_device_info.add_widget(self.dev_dff_lbl)
        layout_device_info.add_widget(self.dev_ns_lbl)
        layout_device_info.add_widget(Divider())
    ...
    def _on_pick(self):
        ...
        if self._list_view.options is not None :
            if self._hsd_info_model.selected_device_id != self._list_view.value:
                ...
                self.dev_sn_lbl.text = "- sn: {}".format(self._hsd_info_model.selected_device_info.serial_number)
                self.dev_al_lbl.text = "- alias: {}".format(self._hsd_info_model.selected_device_info.alias)
                self.dev_pn_lbl.text = "- pn: {}".format(self._hsd_info_model.selected_device_info.part_number)
                self.dev_url_lbl.text = "- url: {}".format(self._hsd_info_model.selected_device_info.url)
                self.dev_fwn_lbl.text = "- fw_name: {}".format(self._hsd_info_model.selected_device_info.fw_name)
                self.dev_fwv_lbl.text = "- fw_version: {}".format(self._hsd_info_model.selected_device_info.fw_version)
                self.dev_dfe_lbl.text = "- data_file_ext: {}".format(self._hsd_info_model.selected_device_info.data_file_ext)
                self.dev_dff_lbl.text = "- data_file_format: {}".format(self._hsd_info_model.selected_device_info.data_file_format)
                self.dev_ns_lbl.text = "- n_sensor: {}".format(self._hsd_info_model.selected_device_info.n_sensor)
    ...
    def _reload_device_list(self):
        ...
        if self._hsd_info_model.device_list is not None:
            ...
            self._list_view.options = [("{}) [{}] - {}".format(i,dev.device_info.alias, dev.device_info.part_number), i) for i,dev in enumerate(self._hsd_info_model.device_list)]
        else:
        ...
        if frame_no - self._last_frame >= self.frame_update_count or self._last_frame == 0:
            ...
            if self._hsd_info_model.is_log_manually_stopped is not True:
                ...
                if self._hsd_info_model.is_log_started:
                    ...
                    s_sensor_data = []
                    item_counter = 0
                    for s in self._hsd_info_model.sensor_list:
                        for sd in s.sensor_descriptor.sub_sensor_descriptor:
                            item = [[s.name, sd.sensor_type, str(self._hsd_info_model.sensor_data_counts[s.id][sd.id])] , item_counter]
