...
class HSDInfo():
    ...
    def __init__(self, cli_flags):
        ...
        self.hsd_link = HSDLink("stwin_hsd", self.cli_flags.output_folder)
    ...
    def update_mlc_sensor_list(self):
        if self.selected_device_id is not None and self.mlc_sensor_list is None:
            ...
            #NOTE WIP - @ the moment only ISM330DHCX_MLC sensor and LSM6DSOX_MLC sensor are supported
            for mlc_s in self.mlc_sensor_list:
                if mlc_s.name == 'ISM330DHCX' or mlc_s.name == 'LSM6DSOX':
