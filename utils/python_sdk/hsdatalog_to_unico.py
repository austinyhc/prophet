...
def show_help(ctx, param, value):
    if value and not ctx.resilient_parsing:
        ...
        click.secho("   python hsdatalog_to_unico.py ..\\STWIN_acquisition_examples\\STWIN_00003 -s LSM6DSOX", fg='cyan')
...
@click.option('-s', '--sensor_name', help="Sensor name - default sensor is ISM330DHCX, LSM6DSOX is also supported", default="ISM330DHCX")
...
def hsd_toUnico(acq_folder, output_folder, sensor_name, sample_start, sample_end, use_datalog_tags, out_format, debug):
    ...
    if sensor is not None:
        if sensor.name == "ISM330DHCX" or sensor.name == "LSM6DSOX":
