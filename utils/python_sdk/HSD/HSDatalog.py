...
from HSD_utils.exceptions import *
import HSD_utils.logger as logger
from HSD.model.DeviceConfig import Device, DeviceInfo, Sensor, SensorDescriptor, SubSensorDescriptor
from HSD.model.AcquisitionInfo import AcquisitionInfo
from HSD.utils.cli_interaction import CLIInteraction as CLI
from HSD.utils.file_manager import FileManager
...
class HSDatalog:
    ...
    def __init__(self, acquisition_folder = None):
        if acquisition_folder is not None:
            try:
                ...
                log.error("No DeviceConfig.json file in your Acquisition folder")
                raise
            try:
    ...
    def get_data_stream_tags(self, sensor_name, sub_sensor_type, sample_start = 0, sample_end = -1):
        '''
        ...
        '''
        res = self.get_data_and_timestamps(sensor_name, sub_sensor_type, sample_start, sample_end)
        if res is not None:
            stream_time = res[1]
    ...
    def get_data_and_timestamps(self, sensor_name, sub_sensor_type, sample_start = 0, sample_end = -1, raw_flag = False):
    ...
    def __to_dataframe(self, data, time, ss_desc, sensor_name, sub_sensor_type, labeled = False, sample_start = 0, sample_end = -1):
        if data is not None and time is not None:
            ...
            if len(ss_desc.dimensions_label) == ss_desc.dimensions:
                ...
                if labeled:
                    tags = self.get_data_stream_tags(sensor_name, sub_sensor_type, sample_start, sample_end)
    ...
    def get_dataframe(self, sensor_name, sub_sensor_type, sample_start = 0, sample_end = -1, labeled = False, raw_flag = False):
        ss_desc, ss_stat = self.get_sub_sensor(sensor_name, ss_type=sub_sensor_type)
        res = self.get_data_and_timestamps(sensor_name, sub_sensor_type, sample_start, sample_end, raw_flag)
        if res is not None:
            data, time = res
    ...
    def get_sensor_plot(self, sensor_name, sub_sensor_type, sample_start = 0, sample_end = -1, label=None, subplots=False, raw_flag = False):
        try:
