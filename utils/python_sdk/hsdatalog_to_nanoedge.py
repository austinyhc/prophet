...
from HSD_utils.exceptions import NanoEdgeConversionError
    ...
    def get_df_object(hsd, sensor, sample_start, sample_end):
        for ssd in sensor.sensor_descriptor.sub_sensor_descriptor:
            ...
            while isLastChunk == 0:
                ...
                if chunk_size == 0:
                    isLastChunk = 1
                    log.info("--> {} NanoEdge conversion completed successfully".format(sensor_name))
                    break
                ...
                if df is not None:
                    try:
                        HSDatalogConverter.to_nanoedge_format(output_folder, sensor_name, df, signal_length, signal_increment)
