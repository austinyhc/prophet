...
#include "systypes.h"
    ...
    /**
     * Create  type name for _AMTask. For more information see the AutoModeTask.h file
     * documentation.
     */
    ...
    /**
     * Allocate an object of ::AMTask type. The object must be initialized with the
     * AMTaskInit() method before calling any API functions. This implementation use the
     * singleton design pattern, so it is possible to allocate only one instance for the
     * application.
     *
    ...
    /**
     * Initialize the object internals status. This method must be called after the object
     * allocation and before any other call to the public API.
     *
     ...
     *
     * @param _this [IN] specifies a pointer to the task object.
     * @return TRUE if the auto mode is started and, either it has completed the execution plan
     * or it has been aborted, FALSE otherwise.
     */
