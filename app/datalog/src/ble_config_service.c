...
/* Console Service */
static const uint8_t console_service_uuid[16] = {
    0x1b,
    0xc5,
    0xd5,
    0xa5,
    ...
    0x00,
    0x00};
static const uint8_t console_term_char_uuid[16] = {
    0x1b,
    0xc5,
    0xd5,
    0xa5,
    ...
    0x00,
    0x00};
static const uint8_t console_stderr_char_uuid[16] = {
    0x1b,
    0xc5,
    0xd5,
    0xa5,
...
static const uint8_t config_service_uuid[16] = {
    0x1b,
    0xc5,
    0xd5,
    0xa5,
    ...
    0x00,
    0x00};
static const uint8_t config_tx_char_uuid[16] = {
    0x1b,
    0xc5,
    0xd5,
    0xa5,
    ...
    0x00,
    0x00};
static const uint8_t config_mlc_char_uuid[16] = {
    0x1b,
    0xc5,
    0xd5,
    0xa5,
...
tBleStatus Add_Config_Service(void)
{
    ...
    ret = aci_gatt_add_service(
        UUID_TYPE_128, &service_config_uuid, PRIMARY_SERVICE, 1 + 6, &configServiceHandle);
    ...
    ret = aci_gatt_add_char(
        ...
        UUID_TYPE_128,
        &char_mlc_uuid,
        2 + 8 + 1 /* 2 byte timestamp, 8 MLC registers output, 1 MCL output state */,
        CHAR_PROP_NOTIFY,
        ATTR_PERMISSION_NONE,
...
tBleStatus Add_Console_Service(void)
{
    ...
    ret = aci_gatt_add_service(
        UUID_TYPE_128, &service_console_uuid, PRIMARY_SERVICE, 1 + 6, &consoleServiceHandle);
...
void setConnectable(void)
{
    ...
    uint8_t lowMemory = 0; // Check SD free size only if an acquisition is not ongoing (to
                           // avoid to dirtying the acquisition)
    ...
    if (BC_State.Id == 0
        || BC_State.Id
               == 9) // id == 9 --> unplugged battery id selection known bug (BatteryCharger)
    {
    ...
    status_ble = aci_gap_set_discoverable(
        ADV_IND,
        0,
        0,
        STATIC_RANDOM_ADDR,
...
void Attribute_Modified_CB(uint16_t attr_handle, uint8_t* att_data, uint8_t data_length)
{
    ...
    else if (attr_handle == configTxCharHandle + 2) /* Enabling the generation of the Battery
                                                       voltage only if it's necessary */
    {
        if (att_data[0] == 01)
        {
...
static uint32_t BLE_ConfigConsole_Command_Parsing(uint8_t* att_data, uint32_t len)
{
    ...
    if (HSCommandBufLen > 0)
    {
        ...
        else if ((outCommand.command == COM_COMMAND_START) && (SD_Logging_Active == 0))
        {
            ...
            else if (BSP_SD_IsDetected() == SD_PRESENT) /* No valid AutoMode configuration,
                                                           usual procedure */
            {
            ...
            ((outCommand.command == COM_COMMAND_STOP) && (SD_Logging_Active == 1))
            || AMTIsStarted())
        {
            COM_AcquisitionDescriptor_t* pAcquisitionDescriptor;
            pAcquisitionDescriptor = COM_GetAcquisitionDescriptor();
...
static uint32_t BLE_ConfigConsole_SetRequests_Parsing(int8_t request, uint32_t len)
{
    ...
    case COM_REQUEST_DEVICE_INFO:
    {
        ...
        HSD_JSON_parse_SetDeviceAliasCommand(
            (char*)hs_command_buffer, alias, HSD_DEVICE_ALIAS_LENGTH);
        HSD_JSON_free(hs_command_buffer);
    ...
    case COM_REQUEST_SW_TAG_LABEL:
    {
        ...
        HSD_JSON_parse_UpdateTagLabelCommand(
            (char*)hs_command_buffer, &id, label, HSD_TAGS_LABEL_LENGTH);
        HSD_JSON_free(hs_command_buffer);
    ...
    case COM_REQUEST_ACQ_INFO:
    {
        ...
        HSD_JSON_parse_AcqInfoCommand(
            (char*)hs_command_buffer, name, HSD_ACQ_NAME_LENGTH, notes, HSD_ACQ_NOTES_LENGTH);
        HSD_JSON_free(hs_command_buffer);
    ...
    case COM_REQUEST_MLC_CONFIG:
    {
        ...
        HSD_JSON_parse_MlcConfigCommand(
            (char*)hs_command_buffer, &mlcConfigSize, mlcConfigData, len);
        HSD_JSON_free(hs_command_buffer);
        ...
        if (osMessagePut(
                bleSendThreadQueue_id, COM_REQUEST_SENSORREFRESH | ISM330DHCX_Get_Id(), 0)
            != osOK)
        {
            SM_Error_Handler();
        }
    ...
    default:
    {
        ...
        {
            osMessagePut(
                bleSendThreadQueue_id, COM_REQUEST_SENSORREFRESH | outCommand.sensorId, 0);
        }
    }
    }
    return 0;
...
static uint32_t BLE_DebugConsole_CommandParsing(uint8_t* att_data, uint32_t len)
{
    ...
    else if (!strncmp("setName ", (char*)(att_data), 8))
    {
        ...
        if (osMessagePut(
                bleSendThreadQueue_id, BLE_COMMAND_DEBUG_CONSOLE | BLE_SUB_CMD_BOARD_NAME, 0)
            != osOK)
        {
            SM_Error_Handler();
        }
    ...
    {
        if (osMessagePut(
                bleSendThreadQueue_id, BLE_COMMAND_DEBUG_CONSOLE | BLE_SUB_CMD_FW_VERSION, 0)
            != osOK)
        {
            SM_Error_Handler();
        }
    ...
    else if (strncmp("upgradeFw", (char*)(att_data), 9) == 0)
    {
        ...
        if (osMessagePut(
                bleSendThreadQueue_id, BLE_COMMAND_DEBUG_CONSOLE | BLE_SUB_CMD_FOTA_START, 0)
            != osOK)
        {
            SM_Error_Handler();
        }
    ...
    {
        if (osMessagePut(
                bleSendThreadQueue_id, BLE_COMMAND_DEBUG_CONSOLE | BLE_SUB_CMD_INFO, 0)
            != osOK)
        {
            SM_Error_Handler();
        }
...
/**
 * @brief  Write a chunk of the new FW to the flash and notify the app in case of error or if
...
static int32_t BLE_OTA_upgrade_chunk(uint8_t* buffer, uint32_t len)
{
    ...
    {
        if (osMessagePut(
                bleSendThreadQueue_id,
                BLE_COMMAND_DEBUG_CONSOLE | BLE_SUB_CMD_FOTA_COMPLETED,
                0)
            != osOK)
        {
            SM_Error_Handler();
    ...
    {
        if (osMessagePut(
                bleSendThreadQueue_id, BLE_COMMAND_DEBUG_CONSOLE | BLE_SUB_CMD_FOTA_ERROR, 0)
            != osOK)
        {
            SM_Error_Handler();
        }
...
void APP_UserEvtRx(void* pData)
{
    ...
    if (hci_pckt->type == HCI_EVENT_PKT)
    {
        ...
        if (event_pckt->evt == EVT_LE_META_EVENT)
        {
            ...
            for (i = 0;
                 i
                 < (sizeof(hci_le_meta_events_table) / sizeof(hci_le_meta_events_table_type));
                 i++)
            {
...
uint32_t SaveBoardNameToFlash(uint32_t address)
{
    ...
    {
        ValueToWrite =
            (((uint64_t)BoardName[4]) << (32 + 24)) | (((uint64_t)BoardName[5]) << (32 + 16))
...
void hci_disconnection_complete_event(
    uint8_t Status, uint16_t Connection_Handle, uint8_t Reason)
{
    connected = FALSE;
