...
#define CONFIG_DATA_RANDOM_ADDRESS          (0x80) /**< Stored static random address. Read-only. */
  ...
  ret = aci_gatt_update_char_value(service_handle, dev_name_char_handle, 0,
                                   strlen((char *)BoardName), (uint8_t *)BoardName);
...
tBleStatus Add_Config_Service(void)
{
  ...
  ret =  aci_gatt_add_char(configServiceHandle, UUID_TYPE_128, &char_tx_uuid, MaxBLECharLen,
                           CHAR_PROP_NOTIFY | CHAR_PROP_WRITE_WITHOUT_RESP, ATTR_PERMISSION_NONE, GATT_NOTIFY_ATTRIBUTE_WRITE, 16, 1,
                           &configTxCharHandle);
...
void setConnectable(void)
{
  ...
  uint8_t manuf_data[25] =
  {
    ...
    15, 0xFF, 0x30, 0x00, /*[length][propetary adv.][STMicroelectronics Manufacturer ID]*/
...
static uint32_t BLE_OTA_upgrade_start(uint8_t *payload)
{
  ...
  else
  {
    ...
    {
      int ret = aci_l2cap_connection_parameter_update_req(connection_handle, 10 /* interval_min*/, 10 /* interval_max */, 0 /* slave_latency */,
