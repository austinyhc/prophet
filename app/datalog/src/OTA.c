 ...
 */
int8_t UpdateFWBlueMS(
    uint32_t* SizeOfUpdate, uint8_t* att_data, int32_t data_length, uint8_t WriteMagicNum)
{
    ...
    if (data_length > (*SizeOfUpdate))
    {
        ...
        OTA_PRINTF(
            "OTA something wrong data_length=%ld RemSizeOfUpdate=%ld....\r\nPlease Try "
            "again\r\n",
            data_length,
            (*SizeOfUpdate));
        ReturnValue = -1;
    ...
    else
    {
        ...
        for (Counter = 0; Counter < data_length; Counter += 8)
        {
            ...
            if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, WritingAddress, ValueToWrite)
                == HAL_OK)
            {
                WritingAddress += 8;
            }
        ...
        if (*SizeOfUpdate == 0)
        {
            ...
            if (WriteMagicNum)
            {
                ...
                if (ExpecteduwCRCValue)
                {
                    ...
                    /* Compute the CRC */
                    uwCRCValue = HAL_CRC_Calculate(
                        &CrcHandle, (uint32_t*)OTA_ADDRESS_START, SizeOfUpdateBlueFW >> 2);
                ...
                if (ReturnValue != 1)
                {
                    ...
                    {
                        OTA_PRINTF(
                            "Wrong CRC! Computed=%lx  expected=%lx ... Try again\r\n",
                            uwCRCValue,
                            ExpecteduwCRCValue);
                    }
                }
...
void StartUpdateFWBlueMS(uint32_t SizeOfUpdate, uint32_t uwCRCValue)
{
    ...
    if (HAL_FLASHEx_Erase(&EraseInitStruct, &SectorError) != HAL_OK)
    {
        ...
        User can add here some code to deal with this error.
        SectorError will contain the faulty sector and then to know the code error on this
        sector, user can call function 'HAL_FLASH_GetError()' FLASH_ErrorTypeDef errorcode =
        HAL_FLASH_GetError(); */
        OTA_ERROR_FUNCTION();
    }
    else
