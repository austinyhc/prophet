...
static uint32_t BLE_CM_ConfigConsole_BuildResponse(
    uint32_t comRequest, char** pSerializedJson, int32_t* serializedJsonSize);
...
void BLE_CM_OS_Init(void)
{
    ...
    bleSendPerformanceStatusTim_id =
        osTimerCreate(osTimer(bleSendPerformanceStatusTim), osTimerPeriodic, NULL);
#endif
    ...
    osThreadDef(
        BLE_USER_EVT_PROC_THREAD,
        ble_user_evt_proc_Thread,
        BLE_USER_EVT_PROC_THREAD_PRIO,
        1,
        2400 / 4);
        ...
        if (connected == TRUE)
        {
            if (evt.status == osEventMessage)
            {
                if ((evt.value.v & BLE_COMMAND_MASK) == BLE_COMMAND_HSD_PROTOCOL)
                {
                    ...
                    BLE_CM_ConfigConsole_BuildResponse(
                        subCommand, &pSerializedJson, &serializedJsonSize);

                    if (serializedJsonSize % (MaxBLECharLen - 1) == 0)
                    {
                        wTP_size =
                            (serializedJsonSize / (MaxBLECharLen - 1)) + serializedJsonSize;
                    }
                    else
                    {
                        wTP_size = (serializedJsonSize / (MaxBLECharLen - 1)) + 1
                                 + serializedJsonSize;
                    }
                    ...
                    bufferSize = BLECommand_TP_Encapsulate(
                        pBuffer, (uint8_t*)pSerializedJson, (uint16_t)serializedJsonSize);
...
static uint32_t BLE_CM_DebugConsole_BuildResponse(uint32_t comRequest)
{
    ...
    case BLE_SUB_CMD_FOTA_COMPLETED:
    {
        int8_t response = 1;
        aci_gatt_update_char_value(
            consoleServiceHandle, consoleTermCharHandle, 0, 1, (uint8_t*)&response);
        HAL_Delay(100);
...
static uint32_t BLE_CM_ConfigConsole_BuildResponse(
    uint32_t comRequest, char** pSerializedJson, int32_t* size)
{
    int32_t serializedJsonSize = 0;
    ...
    case COM_REQUEST_DEVICE:
    {
        ...
        {
            serializedJsonSize =
                HSD_JSON_serialize_FWStatus_Logging(pSerializedJson, BSP_SD_IsDetected(), 1);
        }
        else if (SD_Logging_Active == 0)
        {
            COM_Device_t* pDevice = COM_GetDevice();
            serializedJsonSize =
                HSD_JSON_serialize_Device(pDevice, pSerializedJson, SHORT_JSON);
        }
        else
        {
            serializedJsonSize = HSD_JSON_serialize_FWStatus_Logging(
                pSerializedJson, BSP_SD_IsDetected(), SD_Logging_Active);
        }
        break;
    }
    ...
    case COM_REQUEST_STATUS_PERFORMANCE:
    {
        ...
        serializedJsonSize = HSD_JSON_serialize_FWStatus_Performance(
            pSerializedJson, (char*)&BC_State.Name, mV, level, cpu_usage);
        break;
    }
    ...
    {
        serializedJsonSize =
            HSD_JSON_serialize_FWStatus_Network(pSerializedJson, "ssid", "password", "ip");
        break;
    }
    ...
    default:
    {
        if ((comRequest & COM_REQUEST_SENSORREFRESH) == COM_REQUEST_SENSORREFRESH)
        {
            ...
            serializedJsonSize = HSD_JSON_serialize_RefreshSensorStatus(
                sensorId, pSensorStatus, pSerializedJson);
        }
        break;
    }
    }
...
static uint32_t BLE_CM_ConfigConsole_SendBuffer(uint8_t* buffer, uint32_t len)
{
    ...
    while (j < len)
    {
        ...
        ret = aci_gatt_update_char_value(
            configServiceHandle, configTxCharHandle, 0, chunk, (uint8_t*)&buffer[j]);
        if (ret == BLE_STATUS_INSUFFICIENT_RESOURCES)
        {
            osSemaphoreWait(bleCongestionSem_id, osWaitForever);
        }
