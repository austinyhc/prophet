...
/**
 * Callback function called by the AutoMode whe a new configuration is ready.
 *
...
int main(void)
{
    ...
    strcpy(deviceDescriptorInit.partNumber, "STEVAL-STWINKT1B");
    strcpy(deviceDescriptorInit.URL, "www.st.com/stwin");
    strcpy(deviceDescriptorInit.fwName, "FP-SNS-DATALOG1");
...
void SystemClock_Config(void)
{
    ...
    PeriphClkInit.PeriphClockSelection = RCC_PERIPHCLK_SAI1 | RCC_PERIPHCLK_I2C2
                                       ...
                                       | RCC_PERIPHCLK_DFSDM1AUDIO | RCC_PERIPHCLK_RNG;
 ...
 * @retval
 */
void SENSOR_Generic_Data_Ready(
    uint8_t sensorId, uint8_t subSensorId, uint8_t* buf, uint16_t size, double timeStamp)
{
    ...
    else
    {
        ...
        {
            if (samplesToSend < pSubSensorContext->n_samples_to_timestamp
                || pSubSensorContext->n_samples_to_timestamp == 0)
            {
                /* Pass the complete buffer at once since there is enough space before next
                 * Timestamp */
                if (com_status == HS_DATALOG_SD_STARTED)
                {
                    SDM_Fill_Buffer(
                        sensorId, subSensorId, (uint8_t*)buf, samplesToSend * nBytesPerSample);
                }
                else if (com_status == HS_DATALOG_USB_STARTED)
                {
            ...
            else
            {
                 ...
                 * "samplesToSend==pSubSensorContext->n_samples_to_timestamp"), then pass the
                 * TimeStamp, remaining part (if any) is managed in the next iteration of the
                 * loop */
                if (com_status == HS_DATALOG_SD_STARTED)
                {
                    SDM_Fill_Buffer(
                ...
                {
                    newTS = timeStamp
                          - ((1.0 / (double)pSubSensorStatus->measuredODR) * samplesToSend);
                }
                else
                {
                    newTS = timeStamp;
                ...
                {
                    USBD_WCID_STREAMING_FillTxDataBuffer(
                        &USBD_Device, pSubSensorStatus->comChannelNumber, (uint8_t*)&newTS, 8);
                }
                pSubSensorContext->n_samples_to_timestamp =
                    pSubSensorStatus->samplesPerTimestamp;
            }
        }
    }
}
