    ...
    else
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            if (ret == MSD_OK)
            {
                ...
                /*
                 *
                 * invalidate the scratch buffer before the next read to get the actual data
    ...
    if (!((uint32_t)buff & 0x3))
    {
        ...
        */
        ...
        SCB_CleanDCache_by_Addr(
            (uint32_t*)alignedAddr, count * BLOCKSIZE + ((uint32_t)buff - alignedAddr));
#endif
...
#if (ENABLE_SD_DMA_CACHE_MAINTENANCE == 1)
            /*
             * invalidate the scratch buffer before the next write to get the actual data
             * instead of the cached one
             */
            SCB_InvalidateDCache_by_Addr((uint32_t*)scratch, BLOCKSIZE);
#endif
