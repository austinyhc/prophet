...
static int8_t WCID_STREAMING_Itf_Control(
    uint8_t isHostToDevice,
    uint8_t cmd,
    uint16_t wValue,
    uint16_t wIndex,
    uint8_t* pbuf,
...
static uint32_t WCID_STREAMING_Itf_SerializeRequest(
    COM_Command_t command, char** serialized_json, uint16_t* size);
static uint32_t WCID_STREAMING_Itf_ParseSetRequest(
    COM_Command_t request, char* serialized_json, uint16_t size);
    ...
    uint16_t length)
{
    ...
    if (isHostToDevice)
    {
        ...
        case USBD_WCID_WAITING_FOR_SIZE:
            ...
            serialized =
                HSD_malloc(USB_packet_size); /* Allocate the buffer to receive next command */
            if (serialized == NULL)
            {
                HSD_PRINTF(
                    "Mem alloc error [%d]: %d@%s\r\n", USB_packet_size, __LINE__, __FILE__);
            }
            else
            {
                HSD_PRINTF(
        ...
        case USB_WCID_WAITING_FOR_DATA:
        {
            ...
            if (counter == 0) /* The complete message has been received */
            {
                ...
                {
                    WCID_STREAMING_Itf_ParseSetRequest(
                        outCommand, serialized, USB_packet_size);
        ...
        for (ssID = 0; ssID < pSensorDescriptor->nSubSensors; ssID++)
        {
            ...
            if (pSubSensorStatus->comChannelNumber != -1 && pSubSensorStatus->isActive)
            {
                ...
                TxBuffer[pSubSensorStatus->comChannelNumber] =
                    HSD_calloc((pSubSensorStatus->usbDataPacketSize * 2 + 2), sizeof(uint8_t));

                if (TxBuffer[pSubSensorStatus->comChannelNumber] == NULL)
                {
                    HSD_PRINTF(
                        "Mem alloc error [%d]: %d@%s\r\n",
                        (pSubSensorStatus->usbDataPacketSize * 2 + 2),
                        __LINE__,
                        __FILE__);
                    /* Error */
                    WCID_Error_Handler();
                ...
                USBD_WCID_STREAMING_SetTxDataBuffer(
                    &USBD_Device,
                    pSubSensorStatus->comChannelNumber,
                    TxBuffer[pSubSensorStatus->comChannelNumber],
                    pSubSensorStatus->usbDataPacketSize);
                USBD_WCID_STREAMING_CleanTxDataBuffer(
                    &USBD_Device, pSubSensorStatus->comChannelNumber);
    ...
    COM_Command_t command, char** serialized_json, uint16_t* size)
{
    ...
    {
        pAcquisitionDescriptor = COM_GetAcquisitionDescriptor();
        *size = HSD_JSON_serialize_Acquisition(
            pAcquisitionDescriptor, serialized_json, PRETTY_JSON);
        break;
    }
    case COM_REQUEST_DESCRIPTOR:
    {
        if (command.subSensorId
            < 0) /* Request is for Sensor, since subSensor was not present in the Json */
        {
        ...
        {
            pSubSensorDescriptor =
                COM_GetSubSensorDescriptor(command.sensorId, command.subSensorId);
            *size =
                HSD_JSON_serialize_SubSensorDescriptor(pSubSensorDescriptor, serialized_json);
        }
        break;
    }
    case COM_REQUEST_STATUS:
    {
            ...
            < 0) /* Request is for Sensor, since subSensor was not present in the Json */
        {
            ...
            *size = HSD_JSON_serialize_SensorStatus(
                command.sensorId, pSensorStatus, serialized_json);
        }
        else
        {
    ...
    COM_Command_t command, char* serialized_json, uint16_t size)
{
    ...
    case COM_REQUEST_DEVICE_INFO:
    {
        ...
        HSD_JSON_parse_SetDeviceAliasCommand(
            (char*)serialized_json, alias, HSD_DEVICE_ALIAS_LENGTH);
    ...
    case COM_REQUEST_SW_TAG_LABEL:
    {
        ...
        HSD_JSON_parse_UpdateTagLabelCommand(
            (char*)serialized_json, &id, label, HSD_TAGS_LABEL_LENGTH);
        HSD_JSON_free(serialized_json);
    ...
    case COM_REQUEST_ACQ_INFO:
    {
        ...
        HSD_JSON_parse_AcqInfoCommand(
            (char*)serialized_json, name, HSD_ACQ_NAME_LENGTH, notes, HSD_ACQ_NOTES_LENGTH);
        HSD_JSON_free(serialized_json);
    ...
    case COM_REQUEST_MLC_CONFIG:
    {
        ...
        HSD_JSON_parse_MlcConfigCommand(
            (char*)serialized_json, &mlcConfigSize, mlcConfigData, size);
        HSD_JSON_free(serialized_json);
