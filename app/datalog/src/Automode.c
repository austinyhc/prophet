...
sys_error_code_t AMLoadCfgFromString(const char* pcSerializedCfg)
{
    ...
    // all the other parameters are optional. If not present then the system uses the default
    // value from the AMReset() function.

    // check for the "info.auto_mode" boolean value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.auto_mode", JSONBoolean))
    {
    ...
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.phases_iteration", JSONNumber))
    {
        pxModel->nPhasesIteration =
            (uint16_t)json_object_dotget_number(pxJObj, "info.phases_iteration");
    }

    // check for the "info.start_delay_ms" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.start_delay_ms", JSONNumber))
    {
        pxModel->nStartDelayMS =
            (uint32_t)json_object_dotget_number(pxJObj, "info.start_delay_ms");
    }

    // check for the "info.learning.timer_ms" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.learn.timer_ms", JSONNumber))
    {
        pxModel->xLeanrCfg.nTimerMS =
            (uint32_t)json_object_dotget_number(pxJObj, "info.learn.timer_ms");
    }

    // check for the "info.learning.signals" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.learn.signals", JSONNumber))
    {
        pxModel->xLeanrCfg.nSignals =
            (uint32_t)json_object_dotget_number(pxJObj, "info.learn.signals");
    }

    // check for the "info.detection.timer_ms" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.detect.timer_ms", JSONNumber))
    {
        pxModel->xDetecCfg.nTimerMS =
            (uint32_t)json_object_dotget_number(pxJObj, "info.detect.timer_ms");
    }

    // check for the "info.detection.signals" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.detect.signals", JSONNumber))
    {
        pxModel->xDetecCfg.nSignals =
            (uint32_t)json_object_dotget_number(pxJObj, "info.detect.signals");
    }

    // check for the "info.detection.threshold" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.detect.threshold", JSONNumber))
    {
        pxModel->xDetecCfg.nThreshold =
            (uint8_t)json_object_dotget_number(pxJObj, "info.detect.threshold");
    }

    // check for the "info.detection.sensitivity" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.detect.sensitivity", JSONNumber))
    {
        pxModel->xDetecCfg.fSensitivity =
            (float)json_object_dotget_number(pxJObj, "info.detect.sensitivity");
    }

    // check for the "info.datalog.timer_ms" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.datalog.timer_ms", JSONNumber))
    {
        pxModel->xDatalogCfg.nTimerMS =
            (uint32_t)json_object_dotget_number(pxJObj, "info.datalog.timer_ms");
    }

    // check for the "info.idle.timer_ms" number value
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.idle.timer_ms", JSONNumber))
    {
        pxModel->xIdleCfg.nTimerMS =
            (uint32_t)json_object_dotget_number(pxJObj, "info.idle.timer_ms");
    }

    // check for the "info.execution_plan"
    if ((xRes == SYS_NO_ERROR_CODE)
        && json_object_dothas_value_of_type(pxJObj, "info.execution_plan", JSONArray))
    {
        ...
        for (int j = nExecutionSteps; j < AMC_CFG_MAX_EXECUTION_STEPS; ++j)
        {
