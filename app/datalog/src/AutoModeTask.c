...
#define AM_CMD_ID_NEW_CFG                                                                     \
    ((uint16_t)0x0001) ///< NEW_CFG command ID. For more information see
                       ///< AMTaskOnNewConfiguration() function.
#define AM_CMD_ID_START_EP                                                                    \
    ((uint16_t)0x0002) ///< START_EP command ID. Start the execution phase specified in the
                       ///< parameter of the command.
#define AM_CMD_ID_END_OF_EP                                                                   \
    ((uint16_t)0x0003) ///< END_OF_EP command ID. Notify the task that an execution phase is
                       ///< ended.
#define AM_CMD_ID_ABORT_AM                                                                    \
    ((uint16_t)0x0004) ///< ABORT_AM command ID. Notify the task that the auto mode has been
                       ///< aborted.
#define AM_CMD_ID_RESTART_PLAN                                                                \
    ((uint16_t)0x0005) ///< RESTART_PLAN command ID. Notify the task restart the execution
                       ///< plan.

#define AM_TASK_START_DELAY_MSG                                                               \
    "Auto mode enabled. Execution plan will start after %ld ms.\r\n\r\n"
 ...
 *
 * @param pParams [IN] specifies a pointer to the task object. It is safe to cast as
 * (AMTask*)pParams
 */
 ...
 * @return the execution phase in the plan at the specified index, or E_AM_PHASE_NONE in case
 * of error.
 */
static EAMExecutionPhase AMTaskGetExecutoinPhaseFromPlan(uint8_t nIndex);

/**
 * Start an execution phase. This method look in the execution plan for an execution phase at a
 * given index. It it exist and it is valid, then a command is queued in the task queue in
 * order to start the execution phase at the next step.
 *
...
/**
 * Start the learning execution phase according to the parameters specified in the auto mode
 * model.
 * @param _this [IN] specifies a pointer to the task object.
 * @return SYS_NO_ERROR_CODE if success, an error code otherwise,
 */
...
/**
 * Start the detection execution phase according to the parameters specified in the auto mode
 * model.
 * @param _this [IN] specifies a pointer to the task object.
 * @return SYS_NO_ERROR_CODE if success, an error code otherwise,
 */
...
/**
 * Start the datalog execution phase according to the parameters specified in the auto mode
 * model.
 * @param _this [IN] specifies a pointer to the task object.
 * @return SYS_NO_ERROR_CODE if success, an error code otherwise,
 */
...
sys_error_code_t AMTaskInit(AMTask* _this)
{
    ...
    xRes = xTaskCreate(
        AMTaskRun,
        "AMT",
        AM_TASK_CFG_STACK_DEPTH,
        s_pxTaskObj,
        AM_TASK_CFG_PRIORITY,
        &s_pxTaskObj->m_xTask);
    if (pdPASS == xRes)
    {
        // create the input queue
        _this->m_xInQueue =
            xQueueCreate(AM_TASK_CFG_IN_QUEUE_ITEM_COUNT, AM_TASK_CFG_IN_QUEUE_ITEM_SIZE);
        ...
        else
        {
            ...
            {
                _this->m_xIdleTimer =
                    xTimerCreate("AMter", 1, pdFALSE, _this, AMTaskIdleTimerCallback);
...
sys_error_code_t AMTaskStartExecutioPlan(AMTask* _this)
{
    ...
    if (AMGetIstance()->bIsValid)
    {
        ...
        // mode has been already executed
        if (SYS_IS_CALLED_FROM_ISR())
        {
...
static void AMTaskRun(void* pParams)
{
    ...
    sys_error_code_t xRes = SYS_NO_ERROR_CODE;
    AMCommand xCommand = {0};
    TickType_t xTimeoutInTick = AM_TICK_TO_WAIT_IN_CMD_LOOP;
    AutoModeCfg* pxAutoModeCfg = NULL;

    // we need to wait the start of the console_app otherwise the printf message are not
    // displayed. The console_app has a passive delay of 400msplus an active delay of 1000 ms
    // to start the USB.
    vTaskDelay(500);
            ...
            case AM_CMD_ID_NEW_CFG:
                if (pxTask->m_eState == E_AM_STATE_NOT_STARTED)
                {
                    ...
                    if (pxAutoModeCfg->bIsValid && pxAutoModeCfg->bEnable)
                    {
                        ...
                        // notify the user
                        //              printf(AM_TASK_START_DELAY_MSG,
                        //              pxAutoModeCfg->nStartDelayMS);
                        ...
                        else
                        {
                            ...
                            xRes =
                                AMTaskStartExecutionPhase(pxTask, pxTask->m_nExecutionPlanIdx);
                        }
                    }
                }
                break;
...
static EAMExecutionPhase AMTaskGetExecutoinPhaseFromPlan(uint8_t nIndex)
{
    if (nIndex < AMC_CFG_MAX_EXECUTION_STEPS)
    {
        ...
        if (!strncmp(
                pcExecutionPhase,
                AMC_LEARN_EXECUTION_PHASE_OBJ,
                AMC_CFG_MAX_EXECUTION_STEP_SIZE))
        {
            return E_AM_PHASE_LEARNING;
        }
        else if (!strncmp(
                     pcExecutionPhase,
                     AMC_DETECT_EXECUTION_PHASE_OBJ,
                     AMC_CFG_MAX_EXECUTION_STEP_SIZE))
        {
            return E_AM_PHASE_DETECTION;
                     ...
                     pcExecutionPhase,
                     AMC_DATALOG_EXECUTION_PHASE_OBJ,
                     AMC_CFG_MAX_EXECUTION_STEP_SIZE))
        {
...
static sys_error_code_t AMTaskStartIdle(AMTask* _this)
{
    ...
    if (nIdlePeriodMS)
    {
        ...
        xRes = xTimerChangePeriod(
            _this->m_xIdleTimer, pdMS_TO_TICKS(nIdlePeriodMS), pdMS_TO_TICKS(100));
    }
    else
    {
        // when the idle period is set to zero then I can notify the task the end of the
        // execution phase.
