...
static stmdev_ctx_t MLC_ctx_instance = {
    SM_SPI_Write_Os, SM_SPI_Read_Os, &ism330dhcx_hdl_instance};
...
void SDM_CalculateSdWriteBufferSize(
    COM_SubSensorStatus_t* pSubSensorStatus, uint32_t nBytesPerSample);
...
static void SDM_Thread(void const* argument)
{
    ...
    for (;;)
    {
        ...
        if (com_status == HS_DATALOG_IDLE || com_status == HS_DATALOG_SD_STARTED)
        {
            ...
            if (HAL_GetTick() - SDTick
                > 60000) /* Check SD card memory available every minute */
            {
                SDTick = HAL_GetTick();
...
static void SDM_StartAcquisition(void)
{
    ...
    if (BSP_SD_IsDetected())
    {
        ...
        if (s_nTimerPeriodMS)
        {
            ...
            xTimerChangePeriod(
                s_xStopTimer, pdMS_TO_TICKS(s_nTimerPeriodMS), pdMS_TO_TICKS(200));
        }
    }
    else
    {
...
static void SDM_DataReady(osEvent evt)
{
    ...
    if (evt.value.v
        & SDM_DATA_FIRST_HALF_MASK) /* Data available on first half of the circular buffer */
    {
    ...
    {
        SDM_WriteBuffer(
            sID, ssID, (uint8_t*)(pSubSensorContext->sd_write_buffer + buf_size), buf_size);
    }
}

/**
 * @brief  Check if a custom configuration JSON or UCF is available in the root folder of the
 * SD Card
 * @param  None
...
uint8_t checkConfigJson(FILINFO fno)
{
    ...
    FIL FileConfigJSON;
    int checkDeviceConfig = strncmp(
        fno.fname, "DeviceConfig.json", 17); /* Check if JSON name is DeviceConfig.json */

    if (checkDeviceConfig == 0) /* 0 -> Valid file name -> Load JSON */
    {
        if (f_open(&FileConfigJSON, fno.fname, FA_OPEN_EXISTING | FA_READ)
            == FR_OK) /* Open JSON file */
        {
...
uint8_t checkConfigUcf(FILINFO fno)
{
    uint8_t ret = SDM_DEFAULT_CONFIG;
    FIL FileConfigMLC;
    if (f_open(&FileConfigMLC, fno.fname, FA_OPEN_EXISTING | FA_READ)
        == FR_OK) /* Open UCF file */
    {
...
/**
 * @brief  When battery or memory are low, save the last available data and switch off the
 * board
 * @param  None
 * @retval None
 */
    ...
    for (sID = 0; sID < pDeviceDescriptor->nSensor; sID++)
    {
        ...
        for (ssID = 0; ssID < pSensorDescriptor->nSubSensors; ssID++)
        {
            ...
            if (pSubSensorStatus->isActive)
            {
                ...
                pSubSensorContext->sd_write_buffer =
                    HSD_malloc(pSubSensorStatus->sdWriteBufferSize * 2);
                if (pSubSensorContext->sd_write_buffer == NULL)
                {
                    HSD_PRINTF(
                        "Mem alloc error [%ld]: %d@%s\r\n",
                        pSubSensorStatus->sdWriteBufferSize * 2,
                        __LINE__,
                        __FILE__);
                    SDM_Error_Handler();
                }
    ...
    COM_SubSensorStatus_t* pSubSensorStatus, uint32_t nBytesPerSample)
{
    ...
    /* fixedRAM = total miminum data used by the active subSensors. Each subSensor should use
     * at least SDM_MIN_BUFFER_SIZE bytes */
    float fixedRAM = SDM_MIN_BUFFER_SIZE * activeSubSensors;
    /* variableRAM = data space available. Can be assigned to each subSensor in base of its
     * effective baudrate */
    float variableRAM = SDM_BUFFER_RAM_USAGE - fixedRAM;

    if (pSubSensorStatus->ODR
        != 0) /* Setup bufferSize in base of the subSensor ODR and the RAM available */
    {
        ...
        if (bufferSize > SDM_MAX_WRITE_TIME * pSubSensorStatus->ODR * nBytesPerSample)
        {
            ...
            bufferSize =
                SDM_MAX_WRITE_TIME * (uint32_t)(pSubSensorStatus->ODR * nBytesPerSample);

            /* check samplesPerTimestamp is valid (set SDM_MAX_WRITE_TIME seconds of data or
             * clip to 1000) */
            if (pSubSensorStatus->samplesPerTimestamp != 0)
            {
                uint32_t samplesPerTS = (uint32_t)(SDM_MAX_WRITE_TIME * pSubSensorStatus->ODR);
                if (samplesPerTS > 1000)
                {
                    pSubSensorStatus->samplesPerTimestamp = 1000;
                }
                else
                {
...
void SDM_OS_Init(void)
{
    ...
    /*  create the software timer: one-shot timer. The period is changed in the START command
     * execution. */
...
uint8_t SDM_OpenLogErrorFile(const char* name)
{
    ...
    if (f_write(
            &FileLogError,
            "WARNING: possible data loss at samples [seconds]: ",
            50,
            (void*)&byteswritten)
        != FR_OK)
    {
...
/**
 * @brief  Scan SD Card file system to find the latest directory number that includes to the
 * LOG_DIR_PREFIX
 * @param  None
 * @retval
 */
...
/**
 * @brief  Open one file for each subsensor to store raw data and a JSON file with the device
 * configuration
 * @param  None
 * @retval 1 for f_write error, else 0
 */
    ...
    for (sID = 0; sID < pDeviceDescriptor->nSensor; sID++)
    {
        ...
        for (ssID = 0; ssID < pSensorDescriptor->nSubSensors; ssID++)
        {
            if (COM_GetSubSensorStatus(sID, ssID)->isActive)
            {
                ...
                sprintf(
                    file_name, "%s/%s_%s", dir_name, pSensorDescriptor->name, subSensorName);
                if (SDM_OpenDatFile(sID, ssID, file_name) != 0)
                {
                    return 1;
                }
                activeBaudRate +=
                    COM_GetSubSensorStatus(sID, ssID)->ODR * COM_GetnBytesPerSample(sID, ssID);
                activeSubSensors += 1;
            }
        }
    }
...
static uint32_t SDM_SaveDeviceConfig(char* dir_name)
{
    ...
    if (f_open(&FileConfigHandler, (char const*)file_name, FA_CREATE_ALWAYS | FA_WRITE)
        != FR_OK)
    {
        return 1;
    }
    size = SDM_CreateJSON(&JSON_string);
    if (f_write(&FileConfigHandler, (uint8_t*)JSON_string, size - 1, (void*)&byteswritten)
        != FR_OK)
    {
        return 1;
    }
        ...
        if (fno.fattrib & AM_ARC) /* It is a file. */
        {
            ...
            while (pch != NULL) /* Check files until the end of file list */
            {
                ...
                if (isMLC == 0) /* file name has a 'ucf' extension*/
                {
                    ...
                    int ret = snprintf(file_name, nBuffSize, "%s", dir_name);
                    ...
                    if (f_open(&FileSourceConfigMLC, fno.fname, FA_OPEN_EXISTING | FA_READ)
                        == FR_OK) /* Open UCF file */
                    {
...
uint8_t SDM_Flush_Buffer(uint8_t sID, uint8_t ssID)
{
    ...
    if (pSubSensorContext->sd_write_buffer_idx > 0
        && pSubSensorContext->sd_write_buffer_idx < (bufSize - 1))
    {
        ...
        ret = SDM_WriteBuffer(
            ...
            ssID,
            pSubSensorContext->sd_write_buffer,
            pSubSensorContext->sd_write_buffer_idx + 1);
    }
    else if (
        pSubSensorContext->sd_write_buffer_idx > (bufSize - 1)
        && pSubSensorContext->sd_write_buffer_idx < (bufSize * 2 - 1))
    {
        ...
        ret = SDM_WriteBuffer(
            sID,
            ssID,
            (uint8_t*)(pSubSensorContext->sd_write_buffer + bufSize),
...
uint8_t SDM_Fill_Buffer(uint8_t sID, uint8_t ssID, uint8_t* src, uint16_t srcSize)
{
    ...
    if (pSubSensorContext->sd_write_buffer_idx < (dstSize / 2)
        && dstP >= (dstSize / 2)) /* first half full */
    {
        /* unlock write task */
        if (osMessagePut(
                sdThreadQueue_id,
                sID | ssID << 8 | SDM_DATA_READY_MASK | SDM_DATA_FIRST_HALF_MASK,
                0)
            != osOK)
        {
            SDM_Error_Handler();
    ...
    else if (dstP < pSubSensorContext->sd_write_buffer_idx) /* second half full */
    {
        if (osMessagePut(
                sdThreadQueue_id,
                sID | ssID << 8 | SDM_DATA_READY_MASK | SDM_DATA_SECOND_HALF_MASK,
                0)
            != osOK)
        {
            SDM_Error_Handler();
...
uint32_t SDM_ReadJSON(char* serialized_string)
{
    ...
    {
        update_sensorStatus(
            &local_device->sensors[ii]->sensorStatus,
            &JSON_device.sensors[ii]->sensorStatus,
            ii);
    }
