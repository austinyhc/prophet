...
void update_sensorStatus(
    COM_SensorStatus_t* oldSensorStatus, COM_SensorStatus_t* newSensorStatus, uint8_t sID)
{
    ...
    for (int i = 0; i < COM_GetSubSensorNumber(sID); i++)
    {
        ...
        {
            oldSensorStatus->subSensorStatus[i].FS =
                newSensorStatus->subSensorStatus[i].FS; /* Todo Setters and getters */
        ...
        if (sID == HTS221_Get_Id() || sID == LPS22HH_Get_Id())
        {
            if (i == 0)
            {
                if (oldSensorStatus->subSensorStatus[i].ODR
                    != newSensorStatus->subSensorStatus[i].ODR)
                {
                    update_combo_env += 2;
                }
            }
            if (i == 1)
            {
                ...
                {
                    oldSensorStatus->subSensorStatus[0].ODR =
                        newSensorStatus->subSensorStatus[0].ODR; /* Todo Setters and getters */
                    oldSensorStatus->subSensorStatus[1].ODR =
                        newSensorStatus->subSensorStatus[0].ODR; /* Todo Setters and getters */
                }
                else if (update_combo_env == 1)
                {
                    oldSensorStatus->subSensorStatus[0].ODR =
                        newSensorStatus->subSensorStatus[1].ODR; /* Todo Setters and getters */
                    oldSensorStatus->subSensorStatus[1].ODR =
                        newSensorStatus->subSensorStatus[1].ODR; /* Todo Setters and getters */
                }
                else
                {
                    if (newSensorStatus->subSensorStatus[1].ODR
                        != newSensorStatus->subSensorStatus[0].ODR)
                    {
                        oldSensorStatus->subSensorStatus[0].ODR =
                            newSensorStatus->subSensorStatus[0]
                                .ODR; /* Todo Setters and getters */
                        oldSensorStatus->subSensorStatus[1].ODR =
                            newSensorStatus->subSensorStatus[0]
                                .ODR; /* Todo Setters and getters */
                    }
        ...
        else
        {
            if (oldSensorStatus->subSensorStatus[i].ODR
                != newSensorStatus->subSensorStatus[i].ODR)
            {
                oldSensorStatus->subSensorStatus[i].ODR =
                    newSensorStatus->subSensorStatus[i].ODR; /* Todo Setters and getters */
                if (sID == ISM330DHCX_Get_Id())
                {
                    ISM330DHCX_params_loading += 1;
                }
        ...
        if (oldSensorStatus->subSensorStatus[i].isActive
            != newSensorStatus->subSensorStatus[i].isActive)
        {
            oldSensorStatus->subSensorStatus[i].isActive =
                newSensorStatus->subSensorStatus[i].isActive; /* Todo Setters and getters */
            if (sID == ISM330DHCX_Get_Id())
            {
                ISM330DHCX_params_loading += 1;
            }
        ...
        if (oldSensorStatus->subSensorStatus[i].comChannelNumber
            != newSensorStatus->subSensorStatus[i].comChannelNumber)
        {
            oldSensorStatus->subSensorStatus[i].comChannelNumber =
                newSensorStatus->subSensorStatus[i]
                    .comChannelNumber; /* Todo Setters and getters */
        }

        if (sID == ISM330DHCX_Get_Id()
            && i == 2) /* MLC subsensor: samplesPerTimestamp must be 1 */
        {
        ...
        else
        {
            ...
            uint32_t newSamplesPerTS =
                (uint32_t)(newSensorStatus->subSensorStatus[i].samplesPerTimestamp);
            ...
            if (newSamplesPerTS > idealSamplesPerTS && newSamplesPerTS != 0)
            {
                ...
                {
                    oldSensorStatus->subSensorStatus[i].samplesPerTimestamp =
                        idealSamplesPerTS;
                }
            }
            else
            {
                oldSensorStatus->subSensorStatus[i].samplesPerTimestamp =
                    newSamplesPerTS; /* Todo Setters and getters */
            }
        }

        if (oldSensorStatus->subSensorStatus[i].usbDataPacketSize
            != newSensorStatus->subSensorStatus[i].usbDataPacketSize)
        {
            oldSensorStatus->subSensorStatus[i].usbDataPacketSize =
                newSensorStatus->subSensorStatus[i].usbDataPacketSize;
        }

        if (oldSensorStatus->subSensorStatus[i].sdWriteBufferSize
            != newSensorStatus->subSensorStatus[i].sdWriteBufferSize)
        {
            oldSensorStatus->subSensorStatus[i].sdWriteBufferSize =
                newSensorStatus->subSensorStatus[i].sdWriteBufferSize;
        }

        if (oldSensorStatus->subSensorStatus[i].ucfLoaded
            != newSensorStatus->subSensorStatus[i].ucfLoaded)
        {
            oldSensorStatus->subSensorStatus[i].ucfLoaded =
                newSensorStatus->subSensorStatus[i].ucfLoaded;
        }
    ...
    {
        oldSensorStatus->subSensorStatus[0].sensitivity =
            0.0000305f * oldSensorStatus->subSensorStatus[0].FS;
    }
    else if (sID == IIS2DH_Get_Id())
    {
        ...
        {
            oldSensorStatus->subSensorStatus[0].sensitivity =
                0.00003125f * oldSensorStatus->subSensorStatus[0].FS;
        }
    }
    else if (sID == ISM330DHCX_Get_Id())
    {
        oldSensorStatus->subSensorStatus[0].sensitivity =
            0.0000305f * oldSensorStatus->subSensorStatus[0].FS;
        oldSensorStatus->subSensorStatus[1].sensitivity =
            0.035f * oldSensorStatus->subSensorStatus[1].FS;
    }
#endif
}

void update_sensorStatus_from_USB(
    COM_SensorStatus_t* oldSensorStatus, COM_SensorStatus_t* newSensorStatus, uint8_t sID)
{
    /* Check differences between oldSensorStatus and newSensorStatus, act properly*/
...
void update_samplesPerTimestamp(COM_Sensor_t* pSensor)
{
    for (int sID = 0; sID < pSensor->sensorDescriptor.nSubSensors; sID++)
    {
        ...
        {
            uint32_t idealSamplesPerTS =
                (uint32_t)(pSensor->sensorStatus.subSensorStatus[sID].ODR);
            ...
            {
                pSensor->sensorStatus.subSensorStatus[sID].samplesPerTimestamp =
                    idealSamplesPerTS;
            }
        }
    }
