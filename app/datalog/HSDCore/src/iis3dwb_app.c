...
sensor_handle_t iis3dwb_hdl_instance = {
    IIS3DWB_ID,
    0,
    IIS3DWB_SPI_CS_GPIO_Port,
    IIS3DWB_SPI_CS_Pin,
...
void IIS3DWB_OS_Init(void)
{
    ...
    osThreadDef(
        IIS3DWB_Acquisition_Thread,
        IIS3DWB_Thread,
        HSD_IIS3DWB_THREAD_PRIO,
        1,
        ...
        else if (IIS3DWB_Sensor_State == SM_SENSOR_STATE_RUNNING)
        {
                ...
                == SM_SENSOR_STATE_RUNNING) /* Change of state can happen while task blocked */
            {
                ...
                if ((reg[1]) & 0x80 && (fifo_level >= iis3dwb_samples_per_it))
                {
                    uint16_t i = 0;
                    iis3dwb_read_reg(
                        &iis3dwb_ctx_instance,
                        IIS3DWB_FIFO_DATA_OUT_TAG,
                        (uint8_t*)iis3dwb_mem,
...
static void IIS3DWB_Sensor_Init(void)
{
    ...
    iis3dwb_wtm_level =
        ((uint16_t)IIS3DWB_Init_Param.ODR[0] * (uint16_t)IIS3DWB_MAX_DRDY_PERIOD);
...
__weak void IIS3DWB_Data_Ready(
