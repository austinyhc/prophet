...
const uint16_t TAG_PIN[HSD_TAGS_MAX_HW_CLASSES] = {
    STMOD_PIN_7_PIN, STMOD_PIN_8_PIN, STMOD_PIN_9_PIN, STMOD_PIN_10_PIN, STMOD_PIN_11_PIN};
...
osTimerDef(
    hw_tags,
    HSD_TAGS_TimerCallback); /* when the timer expires, the function toggle_power is called */
    ...
    COM_Device_t* device, HSD_Tags_Type_t type, uint8_t class_id, const char* label)
{
    ...
    label_len = (label_len < HSD_TAGS_LABEL_LENGTH)
                  ? label_len
                  : (HSD_TAGS_LABEL_LENGTH - 1); /* truncate */
        ...
        for (i = 0; i < HSD_TAGS_MAX_HW_CLASSES; i++)
        {
            if (pTagList->HwTag[i].enabled)
            {
                ...
                if (newStatus != TAG_Pin_Status[i])
                {
                    ...
                    HSD_TAGS_add_tag(
                        HSD_TAGS_Type_Hw, i, (HSD_Tags_Enable_t)newStatus, timestamp);
                }
            }
        }
    }
...
void update_tagList(COM_Device_t* oldTagDevice, COM_Device_t* newTagDevice)
{
    ...
    for (int id = 0; id < HSD_TAGS_MAX_HW_CLASSES; id++)
    {
    ...
    for (int id = 0; id < HSD_TAGS_MAX_SW_CLASSES; id++)
    {
