...
/**
 * @brief  SPI thread: it waits on the SPI request queue, performs SPI transactions in non
 * blocking mode and unlocks the thread which made the request at the end of the read.
 * @param  argument not used
 * @retval None
 */
static void spi_Thread(void const* argument)
{
    ...
    for (;;)
    {
        ...
        HAL_GPIO_WritePin(
            ((sensor_handle_t*)msg->sensorHandler)->GPIOx,
...
static void SM_I2C_Init(void)
{
    ...
    HAL_I2C_RegisterCallback(
        &hsm_i2c, HAL_I2C_MEM_RX_COMPLETE_CB_ID, SM_I2C_MemRxCpltCallback);
    HAL_I2C_RegisterCallback(
        &hsm_i2c, HAL_I2C_MEM_TX_COMPLETE_CB_ID, SM_I2C_MemTxCpltCallback);
...
int32_t SM_I2C_Read(void* handle, uint8_t reg, uint8_t* data, uint16_t len)
{
    ...
    HAL_I2C_Mem_Read(
        &hsm_i2c,
        ((sensor_handle_t*)handle)->I2C_address,
        (uint16_t)reg | autoInc,
...
/**
 * @brief  I2C thread: it waits on the I2C request queue, performs I2C transactions in non
 * blocking mode and unlocks the thread which made the request at the end of the read.
 * @param  argument not used
 * @retval None
 */
    ...
    for (;;)
    {
        ...
        if (msg->isRead)
        {
            HAL_I2C_Mem_Read_DMA(
                &hsm_i2c,
                ((sensor_handle_t*)msg->sensorHandler)->I2C_address,
...
double SM_GetTimeStamp(void)
{
    ...
    if (__HAL_TIM_GET_FLAG(
            &hsm_tim,
            TIM_FLAG_UPDATE)) /* Update Event happened while already in critical section */
    {
...
void SM_TIM_Init(void)
{
    ...
    */
    ...
    HAL_TIM_RegisterCallback(
        &hsm_tim, HAL_TIM_PERIOD_ELAPSED_CB_ID, SM_TIM_PeriodElapsedCallback);
