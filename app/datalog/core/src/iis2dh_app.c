...
static sensor_handle_t iis2dh_hdl_instance = {
    IIS2DH_ID, 0, IIS2DH_SPI_CS_GPIO_Port, IIS2DH_SPI_CS_Pin, &iis2dhReadSem_id};
static stmdev_ctx_t iis2dh_ctx_instance = {
    SM_SPI_Write_Os, SM_SPI_Read_Os, &iis2dh_hdl_instance};
...
void IIS2DH_OS_Init(void)
{
    ...
    /* Thread 1 definition */
    osThreadDef(
        IIS2DH_RD_USR_THREAD,
        IIS2DH_Thread,
        HSD_IIS2DH_THREAD_PRIO,
        1,
    ...
    for (;;)
    {
        ...
        else if (IIS2DH_Sensor_State == SM_SENSOR_STATE_RUNNING)
        {
                ...
                == SM_SENSOR_STATE_RUNNING) /* Change of state can happen while task blocked */
            {
                ...
                iis2dh_read_reg(
                    &iis2dh_ctx_instance, IIS2DH_OUT_X_L, (uint8_t*)iis2dh_mem, testWTM * 6);
...
uint8_t IIS2DH_Create_Sensor(const SM_Init_Param_t* pxParams)
{
    ...
    else
    {
        pSensor->sensorStatus.subSensorStatus[0].sensitivity =
            0.00003125f * pSensor->sensorStatus.subSensorStatus[0].FS;
    }
