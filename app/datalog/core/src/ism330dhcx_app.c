...
sensor_handle_t ism330dhcx_hdl_instance = {
    0, 0, ISM330DHCX_SPI_CS_GPIO_Port, ISM330DHCX_SPI_CS_Pin, &ism330dhcxReadSem_id};
stmdev_ctx_t ism330dhcx_ctx_instance = {
...
void ISM330DHCX_sendCMD(int cmd);
...
void ISM330DHCX_Peripheral_Init(void)
{
    ...
    HAL_EXTI_RegisterCallback(
        &ism330dhcx_exti, HAL_EXTI_COMMON_CB_ID, ISM330DHCX_Int_Callback);
...
void ISM330DHCX_OS_Init(void)
{
    ...
    /* Thread 1 definition */
    osThreadDef(
        ISM330_RD_USR_THREAD, ISM330DHCX_Thread, HSD_ISM330DHCX_THREAD_PRIO, 1, 8000 / 4);
    /* Start thread 1 */
...
static void ISM330DHCX_Read_Data(void)
{
    ...
    /* Check FIFO_WTM_IA anf fifo level. We do not use PID in order to avoid reading one
     * register twice */
...
static void ISM330DHCX_Read_Data_From_FIFO(void)
{
    ...
    ism330dhcx_read_reg(
        &ism330dhcx_ctx_instance,
        ISM330DHCX_FIFO_DATA_OUT_TAG,
        (uint8_t*)ism330dhcx_mem,
        ...
        && ISM330DHCX_Init_Param.subSensorActive[1]) /* Read both ACC and GYRO */
    {
        ...
        if (ODR_Acc > ODR_Gyro) /* Acc is faster than Gyro */
        {
            ISM330DHCX_Data_Ready(
                0, (uint8_t*)ism330dhcx_mem, accSamplesCount * 6, TimeStamp_ism330dhcx);
            ISM330DHCX_Data_Ready(
                1, (uint8_t*)ism330dhcx_mem_app, gyroSamplesCount * 6, TimeStamp_ism330dhcx);
        }
        else
        {
            ISM330DHCX_Data_Ready(
                0, (uint8_t*)ism330dhcx_mem_app, accSamplesCount * 6, TimeStamp_ism330dhcx);
            ISM330DHCX_Data_Ready(
                1, (uint8_t*)ism330dhcx_mem, gyroSamplesCount * 6, TimeStamp_ism330dhcx);
        }
    }
    else /* 1 subsensor active only --> simply drop TAGS */
    {
        ...
        if (ISM330DHCX_Init_Param.subSensorActive[0]) /* Acc only */
        {
            ISM330DHCX_Data_Ready(
                0,
                (uint8_t*)ism330dhcx_mem,
                ism330dhcx_samples_per_it * 6,
                TimeStamp_ism330dhcx);
        }
...
static void ISM330DHCX_Read_MLC(void)
{
    if (ISM330DHCX_Init_Param.subSensorActive[2] && MLC_Data_Ready == 1)
    {
        ...
        for (ii = 0; ii < 8; ii++)
        {
            ...
            ism330dhcx_read_reg(
                &ism330dhcx_ctx_instance, ISM330DHCX_MLC0_SRC + ii, (uint8_t*)&mlc_mem[ii], 1);
...
static uint8_t ISM330DHCX_updateSensorStatus(void)
{
    ...
    {
        if ((ISM330DHCX_params_loading != 0)
            && (pSubSensorStatus->isActive == ISM330DHCX_Init_Param.subSensorActive[2]))
        {
    ...
    ISM330DHCX_Init_Param.subSensorActive[0] =
        pSensor->sensorStatus.subSensorStatus[0].isActive;
    ISM330DHCX_Init_Param.subSensorActive[1] =
        pSensor->sensorStatus.subSensorStatus[1].isActive;
    ISM330DHCX_Init_Param.subSensorActive[2] =
        pSensor->sensorStatus.subSensorStatus[2].isActive;
...
static void ISM330DHCX_XL_ODR_From_UCF(void)
{
    ...
    case ISM330DHCX_XL_ODR_OFF:
    {
        ...
        COM_SubSensorStatus_t* pSubSensorStatus =
            COM_GetSubSensorStatus(ISM330DHCX_Get_Id(), 0);
        pSubSensorStatus->isActive = 0;
    }
    break;
...
static void ISM330DHCX_GY_ODR_From_UCF(void)
{
    ...
    case ISM330DHCX_GY_ODR_OFF:
    {
        ...
        COM_SubSensorStatus_t* pSubSensorStatus =
            COM_GetSubSensorStatus(ISM330DHCX_Get_Id(), 1);
        pSubSensorStatus->isActive = 0;
    }
    break;
...
uint8_t ISM330DHCX_updateConfig(void)
{
    ...
    if (pSubSensorStatus->ucfLoaded == 0
        && pSubSensorStatus->isActive
               == 1) /* If you enter here, MLC isActive is true but no UCF are avalable */
    {
...
static void ISM330DHCX_Sensor_Init(void)
{
    ...
    ism330dhcx_wtm_level_acc =
        ((uint16_t)ISM330DHCX_Init_Param.ODR[0] * (uint16_t)ISM330DHCX_MAX_DRDY_PERIOD);
    ism330dhcx_wtm_level_gyro =
        ((uint16_t)ISM330DHCX_Init_Param.ODR[1] * (uint16_t)ISM330DHCX_MAX_DRDY_PERIOD);
 ...
 * @retval 0 --> OK, 1 --> Error
 */
int32_t ISM330DHCX_GetUCF_FromBuffer(
    char* buffer_in, uint32_t size_in, char* buffer_out, uint32_t size_out)
{
    uint8_t* p;
    uint32_t i;
...
__weak void ISM330DHCX_Data_Ready(
...
uint8_t ISM330DHCX_Create_Sensor(const SM_Init_Param_t* pxParams)
{
    ...
    pSensor->sensorStatus.subSensorStatus[0].sensitivity =
        0.0000305f * pSensor->sensorStatus.subSensorStatus[0].FS;
    ...
    pSensor->sensorStatus.subSensorStatus[0].sdWriteBufferSize =
        WRITE_BUFFER_SIZE_ISM330DHCX_A;
    ...
    pSensor->sensorStatus.subSensorStatus[1].sensitivity =
        0.035f * pSensor->sensorStatus.subSensorStatus[1].FS;
    ...
    pSensor->sensorStatus.subSensorStatus[1].sdWriteBufferSize =
        WRITE_BUFFER_SIZE_ISM330DHCX_G;
    ...
    pSensor->sensorStatus.subSensorStatus[2].sdWriteBufferSize =
        WRITE_BUFFER_SIZE_ISM330DHCX_MLC;
