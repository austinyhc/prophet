...
static sensor_handle_t lps22hh_hdl_instance = {
    LPS22HH_ID, LPS22HH_I2C_ADD_H, NULL, 0, &lps22hh_data_read_cmplt_sem_id};
static stmdev_ctx_t lps22hh_ctx_instance = {
    SM_I2C_Write_Os, SM_I2C_Read_Os, &lps22hh_hdl_instance};
...
void LPS22HH_OS_Init(void)
{
    ...
    lps22hh_data_read_cmplt_sem_id =
        osSemaphoreCreate(osSemaphore(lps22hh_data_read_cmplt_sem), 1);
    osSemaphoreWait(lps22hh_data_read_cmplt_sem_id, osWaitForever);

    /* Thread definition: read data */
    osThreadDef(
        LPS22HH_Acquisition_Thread,
        LPS22HH_Thread,
        HSD_LPS22HH_THREAD_PRIO,
        1,
        ...
        else if (LPS22HH_Sensor_State == SM_SENSOR_STATE_RUNNING)
        {
                ...
                == SM_SENSOR_STATE_RUNNING) /* Change of state can happen while task blocked */
            {
                ...
                for (i = 0; i < fifo_level_LPS22HH; i++)
                {
                    ...
                    if (LPS22HH_Init_Param.subSensorActive[0]
                        && !LPS22HH_Init_Param.subSensorActive[1]) /* Only Pressure */
                    {
                        lps22hh_mem_press_f[i] = (float)press / 4096.0f; /* Pressure */
                    }
                    else if (
                        !LPS22HH_Init_Param.subSensorActive[0]
                        && LPS22HH_Init_Param.subSensorActive[1]) /* Only Temperature */
                    {
                        lps22hh_mem_temp_f[i] = (float)temp / 100.0f; /* Temperature */
                    }
                    else if (
                        LPS22HH_Init_Param.subSensorActive[0]
                        && LPS22HH_Init_Param.subSensorActive[1]) /* Both Sub Sensors */
                    {
                        lps22hh_mem_press_f[i] = (float)press / 4096.0f; /* Pressure */
                        lps22hh_mem_temp_f[i] = (float)temp / 100.0f;    /* Temperature */
                    }
                ...
                if (LPS22HH_Init_Param.subSensorActive[0]) /* Press Active */
                {
                    LPS22HH_Data_Ready(
                        0,
                        (uint8_t*)lps22hh_mem_press_f,
                        4 * fifo_level_LPS22HH,
...
#endif
            lps22hh_data_rate_set(
                &lps22hh_ctx_instance, (lps22hh_odr_t)(LPS22HH_POWER_DOWN | 0x10));
...
__weak void LPS22HH_Data_Ready(
...
uint8_t LPS22HH_Create_Sensor(const SM_Init_Param_t* pxParams)
{
    ...
    if (pxParams != NULL)
    {
        ...
        pSensor->sensorStatus.subSensorStatus[1].ODR =
            pSensor->sensorStatus.subSensorStatus[0].ODR;
    }
    else
    {
