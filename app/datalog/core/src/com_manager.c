...
void COM_GenerateAcquisitionUUID(void)
{
     ...
     * N: 1 to 3 most significant bits of digit N represent UUID variant --> using Variant 1
     * (10x)
     * */
    p8[5] = 0x40 | (p8[5] & 0xf);
    ...
    strcpy(
        (char*)COM_device.UUIDAcquisition, (char*)COM_acquisition_descriptor.UUIDAcquisition);
}
...
uint32_t COM_GetnBytesPerSample(uint8_t sID, uint8_t ssID)
{
    ...
    if (pSubSensorDescriptor->dataType == DATA_TYPE_FLOAT
        || pSubSensorDescriptor->dataType == DATA_TYPE_INT32
        || pSubSensorDescriptor->dataType == DATA_TYPE_UINT32)
    {
        return pSubSensorDescriptor->dimensions * 4;
    }
    else if (
        pSubSensorDescriptor->dataType == DATA_TYPE_UINT16
        || pSubSensorDescriptor->dataType == DATA_TYPE_INT16)
    {
        return pSubSensorDescriptor->dimensions * 2;
    }
    else if (
        pSubSensorDescriptor->dataType == DATA_TYPE_UINT8
        || pSubSensorDescriptor->dataType == DATA_TYPE_INT8)
    {
